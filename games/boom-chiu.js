(() => {
  ...
  function drawWeapon(W,H){
    ...
    const rifle=ART?.rifle,aspect=rifle?.naturalWidth&&rifle?.naturalHeight?rifle.naturalWidth/rifle.naturalHeight:1.9,compactLandscape=H<600&&W>H,drawW=Math.min(W*(W<760?.85:compactLandscape?.45:.56),H*.68*aspect),drawH=drawW/aspect,anchorX=W*.78+bobX,anchorY=H*.75+bobY+recoil*11,baseAngle=Math.atan2(anchorY-H*.5,anchorX-W*.5),reloadTilt=reloadActive?Math.sin(reloadPhase*Math.PI)*.72:0,angle=baseAngle+reloadTilt-recoil*.045;
