(() => {
  ...
  // Compatibility namespace: the live Firebase project already allows writes
  // below xiangqiRooms, so Tam Quốc works immediately without a rules deploy.
  ...
  const onlineTab = $('tkOnlineTab');
  const localTab = $('tkLocalTab');
  const lobbyView = $('tkLobbyView');
  const roomDetail = $('tkRoomDetail');
  const nameInput = $('tkOnlineName');
  const createSeat = $('tkCreateSeat');
  const createBtn = $('tkCreateRoom');
  const refreshBtn = $('tkRefresh');
  const roomsEl = $('tkRooms');
  const statusEl = $('tkStatus');
  const roomCodeEl = $('tkRoomCode');
  const roomSubEl = $('tkRoomSub');
  const seatsEl = $('tkOnlineSeats');
  const roomStatusEl = $('tkRoomStatus');
  const leaveBtn = $('tkLeaveRoom');
  const startRoomBtn = $('tkStartRoom');
  const copyRoomBtn = $('tkCopyRoom');
  const botDifficultyEl = $('tkBotDifficulty');
  const specialPiecesEl = $('tkSpecialPieces');
  const hostSettings = $('tkHostSettings');
  ...
  function clientId() {
    let id = readLocal(CLIENT_KEY);
    if (!id) { id = 'tk_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 10); writeLocal(CLIENT_KEY, id); }
    return id;
  }
  ...
  function renderLobby(rows) {
    ...
    rows.forEach(item => {
      ...
      join.className = 'tkRoomJoin'; join.type = 'button'; join.textContent = 'Vào phòng';
      join.addEventListener('click', () => { if (getName()) bindRoom(item.id); });
  ...
  function watchLobby() {
    ...
    // Avoid a new nested .indexOn requirement: room keys are already time ordered.
    lobbyQuery = roomsRef.limitToLast(40);
    lobbyHandler = snap => {
      const rows = [];
      snap.forEach(child => {
        const value = child.val() || {};
        if (value.status === 'waiting' && Date.now() - (Number(value.createdAt) || 0) <= 45 * 60 * 1000) rows.push({ id: child.key, room: value });
      });
      rows.sort((a,b) => (Number(b.room.createdAt)||0) - (Number(a.room.createdAt)||0));
      renderLobby(rows);
    };
    lobbyQuery.on('value', lobbyHandler, () => { roomsEl.innerHTML = '<div class="tkEmpty">Không đọc được lobby. Kiểm tra Firebase.</div>'; });
  }

  function detectMySeat(value) {
    for (let i=0;i<3;i++) { const seat=seatAt(value,i); if (seat.type==='human' && seat.clientId===myClientId) return i; }
    return null;
  }
  function renderRoom(value) {
    room = value;
    mySeat = detectMySeat(value);
    writeLocal(ROOM_KEY, roomId);
    ...
    for (let i=0;i<3;i++) {
      ...
      const who = seat.type === 'human'
        ? ((seat.clientId === myClientId ? 'Bạn · ' : '') + (seat.name || 'Người'))
        : seat.type === 'bot'
          ? '🤖 BOT ' + ((value.botDifficulty||'hard')==='destroyer'?'Hủy diệt':(value.botDifficulty||'hard')==='hard'?'Khó':'Vừa')
          : 'Đang trống';
      card.innerHTML = `<div class="tkSeatHead"><span class="tkSeatDot"></span>${kingdomLabel(i)}</div><div class="tkSeatWho">${who}</div>`;
      const actions = document.createElement('div'); actions.className = 'tkSeatActions';

      if (value.status === 'waiting' && seat.type === 'open') {
        const sit = document.createElement('button'); sit.className='tkSeatBtn'; sit.type='button'; sit.textContent='Ngồi ghế'; sit.addEventListener('click',()=>takeSeat(i)); actions.appendChild(sit);
        if (host) { const bot=document.createElement('button'); bot.className='tkSeatBtn bot'; bot.type='button'; bot.textContent='+ BOT'; bot.addEventListener('click',()=>setSeatBot(i,true)); actions.appendChild(bot); }
      } else if (value.status === 'waiting' && seat.type === 'human' && seat.clientId === myClientId) {
        const mine=document.createElement('span'); mine.className='tkSeatBadge'; mine.textContent='✓ Ghế của bạn'; actions.appendChild(mine);
        const stand=document.createElement('button'); stand.className='tkSeatBtn mine'; stand.type='button'; stand.textContent='Đổi ghế'; stand.addEventListener('click',leaveSeat); actions.appendChild(stand);
      } else if (value.status === 'waiting' && seat.type === 'bot' && host) {
        const remove=document.createElement('button'); remove.className='tkSeatBtn bot'; remove.type='button'; remove.textContent='Bỏ BOT'; remove.addEventListener('click',()=>setSeatBot(i,false)); actions.appendChild(remove);
      } else {
        const fixed=document.createElement('span'); fixed.className='tkSeatBadge'; fixed.textContent=seat.type==='bot'?'🤖 BOT':seat.clientId===myClientId?'✓ Bạn':'👤 Đã có người'; actions.appendChild(fixed);
      }
      card.appendChild(actions); seatsEl.appendChild(card);
    }
    ...
    if (value.status === 'playing' || value.status === 'finished') {
      ...
      setupModal.classList.add('show'); badge.style.display='none';
    }
  }
  function bindRoom(id) {
    if (!roomsRef || !id) return;
    if (roomRef && roomHandler) roomRef.off('value', roomHandler);
    roomId = id; roomRef = roomsRef.child(id);
    roomHandler = snap => {
      const value = snap.val();
      if (!value) { setStatus('Phòng không còn tồn tại.'); leaveRoomView(false); return; }
      renderRoom(value);
    };
  ...
  async function createRoom() {
    ...
    if (pendingInviteRoom) {
      const invited = pendingInviteRoom;
    ...
    createBtn.disabled = true;
    const ref = roomsRef.push(); const now = Date.now();
    const roomSeats = {'0':{type:'open'},'1':{type:'open'},'2':{type:'open'}};
    roomSeats[String(seatIndex)] = {type:'human',name,clientId:myClientId,joinedAt:now};
    try {
      await ref.set({status:'waiting',createdAt:now,updatedAt:now,hostClientId:myClientId,hostName:name,seats:roomSeats,includeSpecial:true,botDifficulty:'hard',state:null,winner:null});
      bindRoom(ref.key);
    } catch (_) { setStatus('Không tạo được phòng. Firebase đang từ chối ghi.'); }
  ...
  async function takeSeat(index) {
    ...
    try {
      await roomRef.transaction(value=>{
        ...
        const target=seatAt(value,index);
        if(target.type!=='open' && !(target.type==='human'&&target.clientId===myClientId))return;
        value.seats=value.seats||{};
        for(let i=0;i<3;i++){const seat=seatAt(value,i);if(seat.type==='human'&&seat.clientId===myClientId)value.seats[String(i)]={type:'open'};}
        value.seats[String(index)]={type:'human',name,clientId:myClientId,joinedAt:Date.now()};
        value.updatedAt=Date.now(); return value;
      },undefined,false);
  ...
  async function leaveSeat(){
    ...
    const index=mySeat;
    try{await roomRef.transaction(value=>{if(!value||value.status!=='waiting')return;const seat=seatAt(value,index);if(seat.type!=='human'||seat.clientId!==myClientId)return;value.seats[String(index)]={type:'open'};value.updatedAt=Date.now();return value;},undefined,false);}catch(_){}
  }
  async function setSeatBot(index,enabled){
    if(!roomRef||!room||room.status!=='waiting'||room.hostClientId!==myClientId)return;
    try{await roomRef.transaction(value=>{if(!value||value.status!=='waiting'||value.hostClientId!==myClientId)return;const seat=seatAt(value,index);if(enabled&&seat.type!=='open')return;if(!enabled&&seat.type!=='bot')return;value.seats=value.seats||{};value.seats[String(index)]=enabled?{type:'bot'}:{type:'open'};value.updatedAt=Date.now();return value;},undefined,false);}catch(_){}
  }
  async function updateHostSettings(){
    if(!roomRef||!room||room.status!=='waiting'||room.hostClientId!==myClientId)return;
    const botDifficulty=botDifficultyEl.value,includeSpecial=!!specialPiecesEl.checked;
    try{await roomRef.transaction(value=>{if(!value||value.status!=='waiting'||value.hostClientId!==myClientId)return;value.botDifficulty=botDifficulty;value.includeSpecial=includeSpecial;value.updatedAt=Date.now();return value;},undefined,false);}catch(_){}
  }
  async function startRoom(){
    if(!roomRef||!room||room.status!=='waiting'||room.hostClientId!==myClientId)return;
    await updateHostSettings();
    try{await roomRef.transaction(value=>{
      if(!value||value.status!=='waiting'||value.hostClientId!==myClientId)return;
      const complete=[0,1,2].every(i=>seatAt(value,i).type==='human'||seatAt(value,i).type==='bot');
      if(!complete)return;
      value.state=R.initialState(value.includeSpecial!==false);value.status='playing';value.startedAt=Date.now();value.updatedAt=Date.now();value.winner=null;return value;
    },undefined,false);}catch(_){setRoomStatus('Không bắt đầu được ván.');}
  }
  function submitMove(move){
    if(!roomRef||!room||room.status!=='playing'||mySeat==null)return;
    const expectedTurn=Number(room.state?.turn);if(expectedTurn!==mySeat)return;
    roomRef.transaction(value=>{
      if(!value||value.status!=='playing'||!value.state)return;
      const turn=Number(value.state.turn);if(turn!==expectedTurn||turn!==mySeat)return;
      const seat=seatAt(value,turn);if(seat.type!=='human'||seat.clientId!==myClientId)return;
      const result=R.makeMove(value.state,move);if(!result.ok)return;
      value.state=result.state;value.updatedAt=Date.now();
      if(result.state.winner!=null){value.status='finished';value.winner=result.state.winner;value.finishedAt=Date.now();}
      return value;
    },(error,committed)=>{if(error||!committed)setRoomStatus('Nước đi không được đồng bộ — thử lại.');},false);
  }
  function maybeDriveBot(value){
    if(!roomRef||value.status!=='playing'||!value.state)return;
    const turn=Number(value.state.turn);
    if(seatAt(value,turn).type!=='bot'){botSignature=null;clearTimeout(botTimer);botTimer=null;Game.setBotThinking(false);return;}
    const signature=String(value.state.moveNo)+':'+turn;if(botSignature===signature)return;
    botSignature=signature;Game.setBotThinking(true);clearTimeout(botTimer);
    botTimer=setTimeout(async()=>{
      botTimer=null;let snap;try{snap=await roomRef.once('value');}catch(_){Game.setBotThinking(false);return;}
      const latest=snap.val();if(!latest||latest.status!=='playing'||!latest.state)return;
      const latestTurn=Number(latest.state.turn);if(String(latest.state.moveNo)+':'+latestTurn!==signature||seatAt(latest,latestTurn).type!=='bot')return;
      const move=Bot.choose(latest.state,latestTurn,latest.botDifficulty||'hard');if(!move)return;
      roomRef.transaction(current=>{
        if(!current||current.status!=='playing'||!current.state)return;
        const currentTurn=Number(current.state.turn);if(String(current.state.moveNo)+':'+currentTurn!==signature||seatAt(current,currentTurn).type!=='bot')return;
        const result=R.makeMove(current.state,move);if(!result.ok)return;
        current.state=result.state;current.updatedAt=Date.now();if(result.state.winner!=null){current.status='finished';current.winner=result.state.winner;current.finishedAt=Date.now();}return current;
      },undefined,false);
    },(value.botDifficulty||'hard')==='destroyer'?700:470);
  }

  function clearRoomParam(){
    try{const url=new URL(location.href);url.searchParams.delete('room');history.replaceState(null,'',url.pathname+url.search+url.hash);}catch(_){}
  }
  function leaveRoomView(showMessage){
    if(roomRef&&roomHandler)roomRef.off('value',roomHandler);
    roomRef=null;roomHandler=null;room=null;roomId=null;mySeat=null;clearTimeout(botTimer);botTimer=null;botSignature=null;
    writeLocal(ROOM_KEY,null);clearRoomParam();lobbyView.style.display='block';roomDetail.classList.remove('show');setupModal.classList.add('show');badge.style.display='none';Game.attachOnline(adapter);if(showMessage)setStatus('Đã rời phòng.');watchLobby();
  }
  async function leaveRoom(){
    if(!roomRef||!room){leaveRoomView(false);return;}
    const ref=roomRef,current=room,seatIndex=mySeat;
    try{
      if(current.status==='waiting'&&current.hostClientId===myClientId)await ref.remove();
      else if(current.status==='waiting'&&seatIndex!=null)await ref.transaction(value=>{if(!value||value.status!=='waiting')return;const seat=seatAt(value,seatIndex);if(seat.type==='human'&&seat.clientId===myClientId){value.seats[String(seatIndex)]={type:'open'};value.updatedAt=Date.now();}return value;},undefined,false);
    }catch(_){}
    leaveRoomView(true);
  }
  function roomShareUrl(id){const url=new URL(location.href);url.searchParams.set('room',id);return url.href;}
  function copyRoomCode(){
    if(!roomId)return;const code=roomId.slice(-5).toUpperCase(),text=roomShareUrl(roomId);
    if(navigator.clipboard?.writeText)navigator.clipboard.writeText(text).then(()=>setRoomStatus('Đã copy link phòng #'+code));
    else setRoomStatus('Link phòng: '+text);
  }
  function restoreSession(){
    if(!roomsRef)return;
    const requestedRoom=new URLSearchParams(location.search).get('room');
    if(requestedRoom){
      if(readLocal(NAME_KEY)){bindRoom(requestedRoom);return;}
      pendingInviteRoom=requestedRoom;
      createBtn.textContent='→ VÀO PHÒNG ĐƯỢC MỜI';
      setStatus('Nhập tên rồi bấm “Vào phòng được mời”.');
      nameInput.focus();
      return;
    }
    const savedRoom=readLocal(ROOM_KEY);if(!savedRoom)return;
    roomsRef.child(savedRoom).once('value').then(snap=>{if(snap.val())bindRoom(savedRoom);else writeLocal(ROOM_KEY,null);}).catch(()=>{});
  }
  ...
  const adapter={
    ...
    backToLobby(){setupModal.classList.add('show');setMode('online');if(roomId&&room)renderRoom(room);},
    requestRematch(){setupModal.classList.add('show');setMode('online');if(roomId&&room)renderRoom(room);}
  };
  ...
  window.ThreeKingdomsOnline={ROOT,createRoom,takeSeat,setSeatBot,startRoom,submitMove,leaveRoom,roomShareUrl,get roomId(){return roomId;},get room(){return room;},get seat(){return mySeat;}};
})();
