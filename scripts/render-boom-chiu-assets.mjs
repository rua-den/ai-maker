...
function orientWeapon(root){
  const {size}=boxInfo(root);
  let from=new THREE.Vector3(1,0,0);
  if(size.y>size.x&&size.y>size.z)from.set(0,1,0);
  else if(size.z>size.x&&size.z>size.y)from.set(0,0,1);
  // Make the rifle's long axis recede into depth and slightly toward upper-left.
  const target=new THREE.Vector3(-.54,.18,-.82).normalize();
  root.quaternion.setFromUnitVectors(from,target);
  root.rotateOnWorldAxis(new THREE.Vector3(0,0,1),-.08);
}
window.renderAsset=async function(kind,url,index=0){renderer.setClearColor(0x000000,0);renderer.clear(true,true,true);const scene=sceneBase(),cam=camera(),root=await load(url);setMaterial(root);normalize(root);if(kind==='soldier'){
  root.rotation.y=index*Math.PI/4;root.rotation.x=-.035;root.position.y=-.08;root.scale.multiplyScalar(1.18);
}else{
  orientWeapon(root);root.position.set(.12,-.16,.18);root.scale.multiplyScalar(1.68);cam.position.set(.28,.24,4.75);cam.lookAt(-.02,-.04,0);
}
scene.add(root);renderer.render(scene,cam);await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));return true}
window.captureCrop=function(pad=kindPad){
  const src=document.createElement('canvas');src.width=720;src.height=720;const s=src.getContext('2d',{willReadFrequently:true});s.drawImage(canvas,0,0);
  const data=s.getImageData(0,0,720,720).data;let minX=720,minY=720,maxX=-1,maxY=-1;
  for(let y=0;y<720;y++)for(let x=0;x<720;x++){if(data[(y*720+x)*4+3]>5){if(x<minX)minX=x;if(x>maxX)maxX=x;if(y<minY)minY=y;if(y>maxY)maxY=y}}
  if(maxX<0)return canvas.toDataURL('image/png');
  minX=Math.max(0,minX-pad);minY=Math.max(0,minY-pad);maxX=Math.min(719,maxX+pad);maxY=Math.min(719,maxY+pad);
  const w=maxX-minX+1,h=maxY-minY+1,out=document.createElement('canvas');out.width=w;out.height=h;out.getContext('2d').drawImage(src,minX,minY,w,h,0,0,w,h);return out.toDataURL('image/png');
}
...
async function shot(kind,url,file,index=0){
  ...
  const dataUrl=await page.evaluate(pad=>window.captureCrop(pad),kind==='soldier'?6:10);
